...
class TelegramResponseSpec extends FreeSpec with Matchers with Utils {

  implicit val formats = DefaultFormats + TelegramResponseSerializer

  s"Extraction of the TelegramResponse from string with $TelegramResponseSerializer" - {
    "when parse response with error" - {
      s"should create instance of $TelegramException" in {
        ...
        // when:
        val error = parse(content).snakizeKeys.extract[TelegramException]
    ...
    "parse empty response" - {
      "should create instance with empty result" in {
        ...
        // when:
        val updates =  parse(content).snakizeKeys.extract[Updates]
    ...
    "parse response with file" - {
      s"should create instance of ${classOf[FileResponse]}" in {
        ...
        // when:
        val response =  parse(content).snakizeKeys.extract[FileResponse]
